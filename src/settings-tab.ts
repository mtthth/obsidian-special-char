...
export class SpecialCharSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Dans les notes en français, marque d'un repère rouge très visible chaque endroit où le français impose une insécable et où elle manque, avant ; ! ? % : et à l'intérieur des guillemets français — que l'espace soit d'un type incorrect (soulignée en plus d'un trait ondulé) ou totalement absente. La commande « Corriger la typographie de la sélection » corrige les deux."
			)
			.addToggle((toggle) =>
