...
const guillemetsNus = [];
plugin.collectWrongSpaces(
	fakeView("«mot»", [{ from: 0, to: 5 }]),
	(from, to, cls) => guillemetsNus.push([from, to, cls]),
	"fr"
);
...
// Un repère, et un seul : la marque posée sur le signe lui-même.
const frenchOnly = [[bang + 1, bang + 2]];

check("seuls les paragraphes français sont signalés", flaggedIn(noteTwo, "fr", true), frenchOnly);
check("sans détection par paragraphe, tout l'est", flaggedIn(noteTwo, "fr", false).length, 2);
...
const oneFault = [[spaceInSpan + 1, spaceInSpan + 2]];
